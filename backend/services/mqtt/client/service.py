    ...
    async def _handle_message(self, message: aiomqtt.Message):
        ...
        try:
            ...
            if mqtt_protocol.should_reset_buffer(end_counter, state.last_packet_num):
                ...
                # Treat as new stream
                await mqtt_data_handler.process_samples(device_id, samples, end_counter, packet_format, sampling_rate)
                return
            ...
            while state.packet_buffer:
                ...
                if is_next or is_full:
                    ...
                    if not is_next and is_full:
                        # Downgrade to debug to avoid spamming logs during network congestion or duplicate streams
                        logger.debug(f"[MQTT] Buffer overflow. Forcing process of {p_start} (Expected {state.last_packet_num + 1}). Gap will be recorded.")

                    # Reparse samples from buffered payload
                    _, p_samples, _, p_format, p_sampling_rate = mqtt_protocol.parse_packet(p_payload)
