...
class MQTTProtocolHandler:
    ...
    def parse_packet(self, payload: dict) -> Tuple[str, List[ECGSample], int, str, int]:
        ...
        if is_batch:
            samples, end_counter, packet_format, sampling_rate = self._parse_batch_packet(payload)
        else:
            samples, end_counter, packet_format, sampling_rate = self._parse_single_packet(payload)
    ...
    def _parse_batch_packet(
        ...
        # Check if payload specifies a rate, otherwise use default
