...
def detect_peaks(signal: np.ndarray, sampling_rate: int) -> Tuple[dict, dict]:
    try:
        clean_signal = nk.ecg_clean(signal, sampling_rate=sampling_rate, method="neurokit")
        signals, rpeaks_info = nk.ecg_peaks(clean_signal, sampling_rate=sampling_rate)
...
def correct_peaks(
        ...
        try:
            ...
            rpeaks_corr['ECG_R_Peaks'] = _clean_array(
            ...
            # Apply corrections
            rpeaks_corr, waves_corr = _correct_first_cycle(
            ...
            rpeaks_corr, waves_corr = _correct_last_cycle(
                rpeaks_corr, waves_corr
            )
...
def _correct_first_cycle(
    ...
    wave_keys = [
        ...
        'ECG_R_Onsets', 'ECG_T_Onsets', 'ECG_P_Onsets'
    ]
