...
from ..signal import signal_processor_5leads as signal_processor
...
class FeatureExtractor:
    ...
    def _extract_lead_ii_features(
        ...
        logger.debug("[FeatureExtractor] Starting _extract_lead_ii_features...")
        features = {
            "rr_avg": 0.0,
            "pr_avg": 0.0,
            "qs_avg": 0.0,
        ...
        try:
            ...
            if len(rpeaks_corr.get("ECG_R_Peaks", [])) == 0:
                logger.debug(
                    "[FeatureExtractor] No R-peaks detected in _extract_lead_ii_features."
                )
                return features
            ...
            logger.debug(
                "[FeatureExtractor] Successfully completed _extract_lead_ii_features."
            )
            return features

        except Exception as e:
            logger.error(
                f"[FeatureExtractor] Unexpected error in _extract_lead_ii_features: {e}"
            )
            return features
    ...
    def _calculate_rr_and_bpm(
        ...
        logger.debug("[FeatureExtractor] Starting _calculate_rr_and_bpm...")
        try:
            ...
            logger.debug(
                "[FeatureExtractor] Successfully completed _calculate_rr_and_bpm."
            )
            return rr_avg, bpm
        except Exception as e:
            logger.error(
                f"[FeatureExtractor] Unexpected error in _calculate_rr_and_bpm: {e}"
            )
            return 0.0, 0.0

    def _calculate_pr_interval(self, waves: dict, sampling_rate: int) -> float:
        logger.debug("[FeatureExtractor] Starting _calculate_pr_interval...")
        try:
            ...
            result = float(sum_val / count_val) if count_val > 0 else 0.0
            logger.debug(
                "[FeatureExtractor] Successfully completed _calculate_pr_interval."
            )
            return result
        except Exception as e:
            logger.error(
                f"[FeatureExtractor] Unexpected error in _calculate_pr_interval: {e}"
            )
            return 0.0

    def _calculate_qs_interval(self, waves: dict, sampling_rate: int) -> float:
        logger.debug("[FeatureExtractor] Starting _calculate_qs_interval...")
        try:
            q_peaks = waves.get("ECG_Q_Peaks", [])
            ...
            result = float(sum_val / count_val) if count_val > 0 else 0.0
            logger.debug(
                "[FeatureExtractor] Successfully completed _calculate_qs_interval."
            )
            return result
        except Exception as e:
            logger.error(
                f"[FeatureExtractor] Unexpected error in _calculate_qs_interval: {e}"
            )
            return 0.0
    ...
    def _calculate_qt_intervals(
        ...
        logger.debug("[FeatureExtractor] Starting _calculate_qt_intervals...")
        try:
            r_onsets = waves.get("ECG_R_Onsets", [])
            ...
            logger.debug(
                "[FeatureExtractor] Successfully completed _calculate_qt_intervals."
            )
            return qt_avg, qtc_avg
        except Exception as e:
            logger.error(
                f"[FeatureExtractor] Unexpected error in _calculate_qt_intervals: {e}"
            )
            return 0.0, 0.0

    def _extract_st_segment(self, signal: np.ndarray, sampling_rate: int) -> float:
        logger.debug("[FeatureExtractor] Starting _extract_st_segment...")
        try:
            filtered = signal_processor.apply_filters(signal, sampling_rate)
            rpeaks, waves = signal_processor.detect_peaks(filtered, sampling_rate)
            ...
            result = float(sum_val / count_val) if count_val > 0 else 0.0
            logger.debug(
                "[FeatureExtractor] Successfully completed _extract_st_segment."
            )
            return result

        except Exception as e:
            logger.error(
                f"[FeatureExtractor] Unexpected error in _extract_st_segment: {e}"
            )
            return 0.0

    def _extract_rs_ratio(self, signal: np.ndarray, sampling_rate: int) -> float:
        logger.debug("[FeatureExtractor] Starting _extract_rs_ratio...")
        try:
            filtered = signal_processor.apply_filters(signal, sampling_rate)
            rpeaks, waves = signal_processor.detect_peaks(filtered, sampling_rate)
            ...
            if (
                ...
                if abs(avg_s) > 1e-9:
                    result = float(avg_r / abs(avg_s))
                    logger.debug(
                        "[FeatureExtractor] Successfully completed _extract_rs_ratio."
                    )
                    return result

            logger.debug(
                "[FeatureExtractor] Successfully completed _extract_rs_ratio with default 0.0."
            )
            return 0.0

        except Exception as e:
            logger.error(
                f"[FeatureExtractor] Unexpected error in _extract_rs_ratio: {e}"
            )
            return 0.0
...
feature_extractor_5leads = FeatureExtractor()
