...
df_resampled = df.resample("5Min").agg({"latency_ms": ["mean", "max"]})
...
ax_plot.fill_between(
    df_resampled.index,
    df_resampled["latency_ms_mean"],
    df_resampled["latency_ms_max"],
    color="#bae6fd",
    alpha=0.5,
    label="Max Spikes (Buffering)",
)

# Styling
ax_plot.set_title(
    "1. Network Latency Endurance",
    fontsize=14,
    fontweight="bold",
    loc="left",
    color="#334155",
...
ax_text.text(
    ...
    bbox=dict(
        boxstyle="round,pad=1", facecolor="#0ea5e9", alpha=0.1, edgecolor="#0ea5e9"
    ),
)
