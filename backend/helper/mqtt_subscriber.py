...
try:
    ...
    print(
        "Warning: Could not import settings from core.config. Using fallback configuration."
    )
...
async def listen():
    print(
        f"[{datetime.now().strftime('%H:%M:%S')}] Connecting to MQTT Broker at {settings.MQTT_BROKER}:{settings.MQTT_PORT}..."
    )
    ...
    try:
        async with aiomqtt.Client(**conn_params) as client:
            topic = "raw/ecg/+"
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] Connected. Subscribing to '{topic}'..."
            )
            await client.subscribe(topic)
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] Subscribed successfully. Listening for messages... (Press Ctrl+C to stop)"
            )
            ...
            async for message in client.messages:
                ...
                try:
                    ...
                    print(
                        f"[{timestamp}] Cnt: {str(cnt):<8} | Topic: {str(message.topic):<20} | Size: {payload_len:<5} | Data: {data_str}"
                    )
                except Exception:
                    # If decoding fails, print raw repr
                    print(
                        f"[{timestamp}] Cnt: {'?':<8} | Topic: {str(message.topic):<20} | Size: {payload_len:<5} | Raw: {message.payload!r}"
                    )
