...
class SessionReader(BaseRepository[TbREcgSession]):
    ...
    def list_by_device(self, device_id: str, limit: int = 100) -> List[TbREcgSession]:
        return self.filter(filters={"device_id": device_id}, limit=limit, order_by="changed_dt", desc_order=True)
        
    def list_by_user(self, user_id: int, limit: int = 100) -> List[TbREcgSession]:
        return self.filter(filters={"user_id": user_id}, limit=limit, order_by="changed_dt", desc_order=True)
    ...
    def get_recent_sessions(self, user_id: int, limit: int) -> List[TbREcgSession]:
        ...
        ).order_by(desc(self.model.changed_dt)).limit(limit).all()
    ...
    def search_sessions(
        ...
        try:
            ...
            if classification:
                query = query.filter(TbREcgSession.classification_result == classification)
            ...
            if start_date:
                # Handle ISO timestamps vs simple dates
