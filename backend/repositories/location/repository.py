...
from models import TbMLocation
...
class LocationRepository(BaseRepository[TbMLocation]):
    ...
    def create(self, data, created_by: str = "SUPERADMIN") -> TbMLocation:
        """
        Accept a LocationCreate Pydantic schema OR a plain dict.
        created_by defaults to SUPERADMIN for router calls that don't pass it explicitly.
        """
        ...
        try:
            ...
            city_slug = (city or name).upper().replace(" ", "")[:8]
            count = (
    ...
    def get_public_list(self) -> List[TbMLocation]:
        """Returns minimal active location list for frontend registration dropdown."""
        logger.debug("[LocationRepository] Starting get_public_list...")
        try:
            return (
                self.db.query(TbMLocation)
                .filter(TbMLocation.is_active.is_(True))
    ...
    def update(
        ...
        """
        Accept a LocationUpdate Pydantic schema OR a plain dict.
