...
class PerformanceReader(BaseRepository[TbRPerformanceLog]):
    ...
    def get_logs_for_recording(self, recording_id: str, limit: int = 1000) -> List[TbRPerformanceLog]:
        return self.filter(
    ...
    def get_logs_for_device(self, device_id: str, hours: int = 24, limit: int = 1000) -> List[TbRPerformanceLog]:
        try:
            ...
            ).order_by(desc(TbRPerformanceLog.created_dt)).limit(limit).all()
        except Exception as e:
            raise DatabaseException(f"Failed to get logs for device {device_id}", details={"error": str(e)})
    ...
    def get_average_metrics_for_recording(self, recording_id: str) -> Dict[str, float]:
        try:
            ...
            ).filter(TbRPerformanceLog.recording_id == recording_id).first()
            
            if not result:
                return {"avg_latency_ms": 0.0, "avg_jitter_ms": 0.0, "avg_packet_loss_pct": 0.0}
            ...
            raise DatabaseException(f"Failed to calculate metrics for {recording_id}", details={"error": str(e)})
    ...
    def get_device_statistics(self, device_id: str, hours: int = 24) -> Dict[str, Any]:
        try:
            ...
            raise DatabaseException(f"Failed to get statistics for device {device_id}", details={"error": str(e)})
    ...
    def get_system_health_summary(self) -> Dict[str, Any]:
        try:
            ...
            ).filter(TbRPerformanceLog.created_dt >= cutoff_time).first()
            
            if not result:
                return {"avg_latency_ms": 0.0, "avg_jitter_ms": 0.0, "avg_packet_loss_pct": 0.0, "active_devices": 0}
            ...
            raise DatabaseException("Failed to get system health summary", details={"error": str(e)})
            
    def get_worst_performing_devices(self, metric: str = "latency", limit: int = 5, hours: int = 24) -> List[Dict[str, Any]]:
        try:
            cutoff_time = datetime.utcnow() - timedelta(hours=hours)
            if metric == "latency":
            ...
            raise DatabaseException("Failed to get worst performing devices", details={"error": str(e)})