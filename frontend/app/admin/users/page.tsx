...
export default function UserManagementPage() {
    ...
    useEffect(() => {
        const total = Math.ceil(users.length / rowsPerPage) || 1;
        if (page > total) {
            setPage(total);
        }
    ...
    return (
        <div className="flex flex-col h-full w-full bg-slate-50/50 p-6 lg:p-8 gap-6 overflow-hidden animate-in fade-in duration-500">
                            ...
                            <table className="w-full text-left border-collapse table-auto h-full">
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {paginatedUsers.map((user) => (
                                        <tr key={user.id} className="hover:bg-rose-50/30 transition-colors group h-[48px]">
                                            ...
                                            <td className="px-5 whitespace-nowrap text-right pr-6">
                                                <div className="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <button 
                                                        ...
                                                        className="px-3 py-1.5 bg-blue-50 text-blue-600 rounded text-[9px] font-black uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all active:scale-95 border border-blue-100/50"
                                                        title="Edit User"
                                                    >
                                                        Edit
                                                    </button>
            ...
            <ConfirmationModal
                ...
                message={`Are you sure you want to permanently delete @${deletingUser?.username}? This action will remove ALL associated data including ECG sessions, patient profiles, and logs. This cannot be undone.`}
