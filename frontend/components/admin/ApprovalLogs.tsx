                ...
                <div ref={containerRef} className="flex-1 overflow-x-auto no-scrollbar relative z-10 overflow-y-hidden">
                    <table className="w-full text-left border-collapse table-fixed lg:table-auto h-full">
                        <thead className="bg-slate-50/50 text-slate-400 sticky top-0 z-10 backdrop-blur-sm h-[48px]">
                            <tr>
                                <th className="px-4 lg:px-6 font-black text-[9px] uppercase tracking-[0.15em] border-b border-slate-100 whitespace-nowrap w-[180px]">
                                    Date & Time
                                </th>
                                <th className="px-4 lg:px-6 font-black text-[9px] uppercase tracking-[0.15em] border-b border-slate-100 whitespace-nowrap">
                                    User Profile
                                </th>
                                <th className="px-4 lg:px-6 font-black text-[9px] uppercase tracking-[0.15em] border-b border-slate-100 whitespace-nowrap w-[140px]">
                                    Status
                                </th>
                                <th className="px-4 lg:px-6 font-black text-[9px] uppercase tracking-[0.15em] border-b border-slate-100 whitespace-nowrap">
                                    Reason / Notes
                                </th>
                                <th className="px-4 lg:px-6 font-black text-[9px] uppercase tracking-[0.15em] border-b border-slate-100 whitespace-nowrap w-[120px]">
                                    Admin
                                </th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                            {paginatedLogs.length === 0 ? (
                                ...
                                paginatedLogs.map((log) => (
                                    <tr key={log.id} className="hover:bg-slate-50/50 transition-colors group h-[48px]">
                                        <td className="px-4 lg:px-6 whitespace-nowrap">
                                            <p className="text-[11px] font-bold text-slate-700">{formatDate(log.created_dt).split(',')[0]}</p>
