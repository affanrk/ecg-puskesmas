...
export default function ApprovalsQueue({ users, searchTerm, dateRange, onApprove, onReject, onViewDetails }: ApprovalsQueueProps) {
        ...
        return (
            <div className="h-full w-full flex flex-col items-center justify-center py-20 border-2 border-dashed border-slate-200 rounded-2xl bg-white/50">
                ...
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">
                    {searchTerm || dateRange.start ? "No results match your filters" : "Verification queue is currently empty"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-4 pb-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {displayUsers.map((user) => (
                <div 
                    key={user.id} 
                    onClick={() => onViewDetails(user)}
                    className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden group hover:border-rose-300 transition-all duration-300 cursor-pointer hover:shadow-md h-fit"
                >
                    <div className="p-6 flex items-start gap-5">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-4">
                                <div>
                                    <h3 className="text-base font-black text-slate-800 truncate leading-tight mb-1 group-hover:text-rose-700 transition-colors">
                    ...
                    <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                        <div className="flex flex-col">
                            ...
                            <span className="text-[10px] font-bold text-slate-500">
                                {formatDate((user.changed_dt || user.created_dt) as string)}
                            </span>
                        </div>
                        <div className="flex items-center gap-3">
                            <span className="text-[9px] font-bold text-slate-300 uppercase tracking-widest mr-2 group-hover:text-rose-400/60 transition-colors hidden sm:block">
                                Click for details
                            </span>
                            <button 
                                onClick={(e) => {
