...
import { ShieldCheck, AlertCircle, Settings2, Clock, Stethoscope } from 'lucide-react';
...
export default function DoctorProfileHeader({ user, isLocked, activeTab, setActiveTab }: DoctorProfileHeaderProps) {
    if (!user) return null;
    ...
    const specialty = getActiveProfile(user)?.specialty;
    ...
    return (
        <div className="bg-white p-4 lg:p-5 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-96 h-96 bg-gradient-to-bl from-rose-50 to-transparent rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 opacity-60 pointer-events-none group-hover:scale-110 transition-transform duration-700"></div>
            <div className="flex items-center gap-6 w-full md:w-auto relative z-10">
                <div className="relative shrink-0">
                    <div className="w-20 h-20 bg-gradient-to-br from-rose-500 to-red-600 rounded-xl flex items-center justify-center text-white shadow-xl shadow-rose-500/20 text-3xl font-black ring-4 ring-slate-50">
                    ...
                    <div className={clsx(
                        ...
                        isActivated ? "bg-rose-500 text-white" : (hasRejection ? "bg-rose-500 text-white" : (isLocked ? "bg-amber-500 text-white" : "bg-slate-400 text-white"))
                    )}>
                ...
                <div className="space-y-1.5 text-center md:text-left">
                    ...
                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 pt-0.5">
                        {specialty && (
                            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-rose-50 text-rose-600 text-[9px] font-bold uppercase tracking-wider border border-rose-100">
                        ...
                        <span className={clsx(
                            "px-2.5 py-1 rounded-md text-[9px] font-bold uppercase tracking-wider border flex items-center gap-1.5 shadow-sm",
                            isActivated ? "bg-rose-50 text-rose-700 border-rose-100" :
                            ...
                            {isActivated ? "Verified Doctor" : (hasRejection ? "Profile Rejected" : (isLocked ? "Awaiting Approval" : "Profile Incomplete"))}
                        </span>
                    </div>
                </div>
            </div>
            <div className="p-1 bg-slate-100/80 rounded-lg w-full md:w-auto relative z-10 backdrop-blur-sm flex gap-1">
                ...
                >
                    <Stethoscope size={14} strokeWidth={2.5} className={activeTab === 'medical' ? "text-rose-500" : "opacity-50"} />
                    Medical Profile
                </button>
                <button
