...
import { SecurityFormPayload } from '@/types/user';

interface SharedPasswordCardProps {
    isChangingPassword: boolean;
    setIsChangingPassword: (val: boolean) => void;
    securityForm: SecurityFormPayload;
    setSecurityForm: React.Dispatch<React.SetStateAction<SecurityFormPayload>>;
...
export default function SharedPasswordCard({
    isChangingPassword,
        ...
        <div className="bg-white p-5 lg:p-6 flex flex-col w-full transition-all duration-500 relative group">
            ...
            <div className="flex items-center justify-between border-b border-slate-50 pb-3 mb-5 relative z-10 shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-base font-black text-slate-800 tracking-tight">Security Access</h2>
                ...
                {!isChangingPassword && (
                    <button
                        ...
                        className="text-[9px] font-black uppercase tracking-widest text-slate-400 hover:text-rose-600 hover:bg-rose-50 px-3 py-2 rounded-md flex items-center gap-2 transition-all duration-300 border border-slate-100 hover:border-rose-200 cursor-pointer"
                    >
                ...
                {!isChangingPassword ? (
                    <div className="flex-1 flex items-center justify-center animate-in fade-in duration-500">
                        <div className="w-full flex flex-col items-center justify-center p-6 bg-slate-50/50 rounded-md border border-dashed border-slate-200 min-h-[200px] group/box transition-all duration-500">
                            <div className="w-16 h-16 bg-white rounded-md shadow-md flex items-center justify-center mb-4 text-emerald-500 ring-1 ring-emerald-100 transition-transform duration-500">
                                <ShieldCheck size={32} strokeWidth={1.5} />
                            </div>
                            <p className="text-slate-900 text-[10px] font-black tracking-tight mb-2 uppercase tracking-widest">Encryption Active</p>
                            <p className="text-slate-500 text-[10px] font-bold text-center max-w-[220px] leading-relaxed mb-4">
                    ...
                    <div className="flex-1 flex flex-col w-full animate-in fade-in duration-300">
                        <div className="space-y-4 flex-1 overflow-y-auto pr-1">
                            ...
                            />
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-4 pt-1">
                                <StandardInput
                                    label="New Password"
                                    type="password"
                        ...
                        <div className="flex flex-wrap gap-3 pt-6 border-t border-slate-50 mt-6 shrink-0">
                            ...
                            <button
                                ...
                                disabled={loading}
                                className="flex-[2] px-6 py-3.5 bg-rose-600 hover:bg-rose-700 disabled:opacity-70 disabled:cursor-not-allowed text-white text-[10px] font-black uppercase tracking-widest rounded-md shadow-xl shadow-rose-500/20 transition-all active:scale-[0.98] flex items-center justify-center gap-3 hover:-translate-y-0.5 cursor-pointer"
                            >
                                {loading ? (
                                    <>
                                    ...
                                    <>
                                        <Key size={16} strokeWidth={2.5} /> Update Password
                                    </>
                                )}
                            </button>
                        </div>
