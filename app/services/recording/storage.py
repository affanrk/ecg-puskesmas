...
from app.repositories.session import SessionRepository, RawDataRepository
from app.repositories.performance import PerformanceRepository
from app.core.database import SessionLocal
from app.utils.constants import DB_BATCH_INTERVAL, DB_BATCH_CHUNK_SIZE
...
class RecordingStorageService:
    ...
    def _execute_batch_insert(
        ...
        try:
            # 1. Insert recording data (ECG signals)
            if rec_items:
                self._insert_recording_batch(db, rec_items)
                
            # 2. Insert performance logs
            if perf_items:
    ...
    def _insert_recording_batch(
        ...
        valid_items = [
            ...
            if item['recording_id'] not in cancelled
        ]
        ...
        # Split into chunks for very large batches
